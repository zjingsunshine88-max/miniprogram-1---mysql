...
:: Clean problematic configuration files
echo [STEP 2] Clean problematic configuration files
echo [INFO] Removing all .conf files from conf.d...
for %%f in (C:\nginx\conf.d\*.conf) do (
    del "%%f" >nul 2>&1
    echo [OK] Removed %%f
)

echo [INFO] Removing problematic .conf files from conf...
for %%f in (C:\nginx\conf\*.conf) do (
    if not "%%f"=="C:\nginx\conf\nginx.conf" (
        del "%%f" >nul 2>&1
        echo [OK] Removed %%f
...
echo [STEP 3] Create clean nginx.conf
(
...
:: Create SSL configuration with port 8443
echo [STEP 4] Create SSL configuration with port 8443
(
echo # SSL configuration with port 8443
echo server {
echo     listen 80;
...
echo [OK] Created SSL configuration with port 8443

:: Create mime.types if not exists
echo [STEP 5] Create mime.types
if not exist "C:\nginx\mime.types" (
    (
        echo types {
...
if %errorlevel% equ 0 (
    ...
    echo [INFO] This indicates a deeper configuration issue
)

:: Check port 80
echo [STEP 11] Check port 80
...
echo.
echo Summary:
echo - Cleaned all problematic configuration files
echo - Created clean nginx.conf with conf.d include
echo - Created SSL configuration with port 8443
echo - Created necessary files (mime.types, logs)
echo - Tested and started nginx
echo.
echo If port 8443 is now listening, you can access:
