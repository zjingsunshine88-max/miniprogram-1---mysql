...
if exist "C:\nginx\conf\nginx.conf" (
    ...
    echo [OK] 已备份nginx.conf
)

:: 步骤3: 删除有问题的配置文件
echo [步骤3] 删除有问题的配置文件...
if exist "C:\nginx\conf\localhost.conf" (
    copy "C:\nginx\conf\localhost.conf" "C:\nginx\conf\localhost.conf.backup" >nul
    del "C:\nginx\conf\localhost.conf"
    echo [OK] 已删除localhost.conf
)

:: 删除其他可能有问题的配置文件
for %%f in (C:\nginx\conf\*.conf) do (
    if not "%%f"=="C:\nginx\conf\nginx.conf" (
        findstr /i "ECHO" "%%f" >nul 2>&1
        if %errorlevel% equ 0 (
            copy "%%f" "%%f.backup" >nul
            del "%%f"
            echo [OK] 已删除 %%f
