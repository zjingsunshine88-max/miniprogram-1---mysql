...
$Host.UI.RawUI.WindowTitle = "使用Python运行Certbot获取SSL证书"

Write-Host "🔐 使用Python运行Certbot获取SSL证书" -ForegroundColor Green
Write-Host ""
...
# 使用Python运行Certbot
try {
    & python -m certbot certonly --manual --preferred-challenges dns -d practice.insightdata.top --email admin@practice.insightdata.top --agree-tos
